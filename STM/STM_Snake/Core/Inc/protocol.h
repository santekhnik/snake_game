...
uint16_t encode_frame_end(const uint8_t *payload, uint8_t payload_len, uint8_t *frame, uint8_t cmd_byte, uint8_t frog_x, uint8_t frog_y) {
    frame[0] = START_BYTE;                  				// Початковий байт
    frame[1] = cmd_byte;                    				// Байт комади
    frame[2] = payload;										//копіюємо пейлоад
