...
uint8_t rxdata;
int x = 0, y = 0; // Початкові координати
char direction = 'd'; // Початковий напрямок руху ('w', 's', 'a', 'd')
int length = 0; // Довжина змійки
int apple_x, apple_y; // Координати яблука
int body_x[MAX_LENGTH];
int body_y[MAX_LENGTH];
int len;
/* USER CODE BEGIN PV */
...
void send_preload_arrays(uint8_t *body_x, uint8_t *body_y, uint8_t length) {
    uint8_t preload[length * 2];  // Масив для об'єднаних даних

    for (uint8_t i = 0; i < length; i++) {
        preload[i * 2] = body_x[i];      // Записуємо елементи з першого масиву
